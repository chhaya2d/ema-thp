...
public final class JoinExecutor {
    ...
    public List<EngineRow> materialize(UserContext user, Map<String, Connector> connectorsByName, JoinQuery jq) {
        Connector leftConn = lookup(connectorsByName, jq.left());
        Connector rightConn = lookup(connectorsByName, jq.right());
        List<EngineRow> leftRows = executeSide(user, leftConn, jq.left(), jq.where());
        List<EngineRow> rightRows = executeSide(user, rightConn, jq.right(), jq.where());
